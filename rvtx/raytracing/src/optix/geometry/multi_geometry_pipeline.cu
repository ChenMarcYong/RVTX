...
{
    GeometryHandler::GeometryHandler( const optix::Context & context ) : m_context( &context ) {}

    MultiGeometryPipeline::MultiGeometryPipeline( const optix::Context & context ) : Pipeline( context )
    {
    ...
    MultiGeometryPipeline::MultiGeometryPipeline( MultiGeometryPipeline && other ) noexcept
    {
    ...
    MultiGeometryPipeline & MultiGeometryPipeline::operator=( MultiGeometryPipeline && other ) noexcept
    {
        std::swap( m_rayGenModule, other.m_rayGenModule );
        std::swap( m_rayGen, other.m_rayGen );
        std::swap( m_missGroup, other.m_missGroup );
    ...
    void MultiGeometryPipeline::compile()
    {
        m_rayGen = {};
        ...
        m_dHitGroupRecord = cuda::DeviceBuffer::Typed<GeometryHitGroupRecord>( m_hitGroupRecordNb );
        ...
        cuda::cudaCheck( cudaMemcpy( m_dHitGroupRecord.get<uint8_t>(),
    ...
    void MultiGeometryPipeline::updateGeometry()
    {
        struct Dummy
        {
        };

        m_dRayGenRecord            = cuda::DeviceBuffer::Typed<optix::Record<Dummy>>( 1u );
        ...
        m_dMissRecord = cuda::DeviceBuffer::Typed<optix::Record<Dummy>>( 1 );
        {
        ...
        auto         dInstances
            = cuda::DeviceBuffer( instancesSizeInBytes + instancesSizeInBytes % OPTIX_INSTANCE_BYTE_ALIGNMENT );
        ...
        m_dIas = cuda::DeviceBuffer( accelBufferSizes.outputSizeInBytes
                                     + accelBufferSizes.outputSizeInBytes % OPTIX_ACCEL_BUFFER_BYTE_ALIGNMENT );

        auto dTmp = cuda::DeviceBuffer( accelBufferSizes.tempSizeInBytes
