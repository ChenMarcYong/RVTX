...
void unpackGBuffers( ivec2 px, out UnpackedData data )
{
	...
	data.discardShading = viewPositionNormal.w == 1;
}
...
void main()
{
	...
	if ( data.viewPosition.z == 0.f && !data.discardShading)
	{
		fragColor = vec4( uBackgroundColor, 1.f );
		return;
	}
	
	vec3 color = texelFetch( gbColor, texCoord, 0 ).xyz;
	if(data.discardShading)
	{
	...
	color *= cosTheta * ambientOcclusion;

	fragColor = vec4( color * uLightColor, 1.f );
}
