...
void main()
{
	...
	const vec3 v0	  = (uIsPerspective ? vec3(0) : viewImpPos) - viewCylVert[ 0 ];
	...
	const float c = d0 * dot( v0, v0 ) - d2 * d2 - uCylRadius * uCylRadius * d0;
	...
	else
	{
		...
		else // Orthographic
		{
			...
			else
			{
				...
				normal = normalize( hit - viewCylVert[ 0 ] - v1v0 * y / d0 );
			}
