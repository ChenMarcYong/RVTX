...
void main()
{
	...
	if (uIsPerspective)
	{
		...
		const float sinAngle = uCylRadius / dV0;
		float		angle	 = asin( sinAngle );
		const vec3	y1		 = y * uCylRadius;
		const vec3	x2		 = x * uCylRadius * cos( angle );
		const vec3	y2		 = y1 * sinAngle;
		angle				 = asin( uCylRadius / dV1 );
		const vec3 x3		 = x * ( dV1 - uCylRadius ) * tan( angle );
	...
	else
	{
		...
		const vec3 vertStep = normalize(vec3(-dirCyl.y, dirCyl.x, 0)) * uCylRadius;
