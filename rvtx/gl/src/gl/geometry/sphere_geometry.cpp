...
namespace rvtx::gl
{
    ...
    SphereHolder SphereHolder::getMolecule( const Molecule & molecule, const MoleculeIDs * moleculeIds )
    {
        SphereHolder holder;
    ...
    SphereHolder SphereHolder::getNonResident( const Molecule & molecule, const MoleculeIDs * moleculeIds )
    {
        SphereHolder holder;
    ...
    SphereHolder SphereHolder::getSystem( const Molecule & molecule, const MoleculeIDs * moleculeIds )
    {
        SphereHolder holder {};
    ...
    SphereHandler::SphereHandler( ProgramManager & manager )
    {
    ...
    SphereHandler::SphereHandler( SphereHandler && other ) noexcept
    {
    ...
    SphereHandler & SphereHandler::operator=( SphereHandler && other ) noexcept
    {
        std::swap( m_program, other.m_program );
        std::swap( m_uniforms, other.m_uniforms );
        std::swap( m_vao, other.m_vao );
    ...
    void SphereHandler::render( const Camera & camera, const Scene & scene )
    {
        m_program->use();
        ...
        const auto entities = scene.registry.view<Molecule, Transform, SphereHolder>();
        for ( const auto id : entities )
        {
            entt::basic_handle entity                    = { scene.registry, id };
            const auto & [ molecule, transform, holder ] = entity.get<Molecule, Transform, SphereHolder>();
