...
namespace rvtx::gl
{
    ...
    BallAndStickHolder BallAndStickHolder::getMolecule( const Molecule &    molecule,
                                                        const MoleculeIDs * moleculeIds,
                                                        const float         radius )
    {
        BallAndStickHolder holder;
        ...
        holder.cylinderRadius    = radius / 4.f;

        return holder;
    }

    BallAndStickHolder BallAndStickHolder::getNonResident( const Molecule &    molecule,
                                                           const MoleculeIDs * moleculeIds,
                                                           const float         radius )
    {
        BallAndStickHolder holder;
    ...
    BallAndStickHolder BallAndStickHolder::getSystem( const Molecule &    molecule,
                                                      const MoleculeIDs * moleculeIds,
                                                      const float         radius )
    {
        BallAndStickHolder holder;
    ...
    BallAndStickHandler::BallAndStickHandler( ProgramManager & manager )
    {
    ...
    BallAndStickHandler::BallAndStickHandler( BallAndStickHandler && other ) noexcept
    {
    ...
    BallAndStickHandler & BallAndStickHandler::operator=( BallAndStickHandler && other ) noexcept
    {
        std::swap( m_sphereProgram, other.m_sphereProgram );
        std::swap( m_cylinderProgram, other.m_cylinderProgram );
        std::swap( m_uniforms, other.m_uniforms );
    ...
    BallAndStickHandler::~BallAndStickHandler()
    {
        if ( glIsVertexArray( m_vao ) )
            glDeleteVertexArrays( 1, &m_vao );
    }

    void BallAndStickHandler::render( const Camera & camera, const Scene & scene )
    {
        ...
        const auto entities = scene.registry.view<Molecule, Transform, BallAndStickHolder>();
        for ( const auto id : entities )
        {
            entt::basic_handle entity                    = { scene.registry, id };
            const auto & [ molecule, transform, holder ] = entity.get<Molecule, Transform, BallAndStickHolder>();
